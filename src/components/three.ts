...
class ParticleExplode extends Base {
  ...
  constructor(sel: string, debug: boolean, canRecovery: boolean) {
    super(sel, debug);
  ...
  createClickEffect() {
    ...
    image.addEventListener("click", () => {
      if (!this.canRecovery && this.isOpen) return;
      if (!this.isOpen) {
...
export const start = (domId: string) => {
  const particleExplode = new ParticleExplode(domId, false, false);
